...
const Devolution = () => {
  ...
  const { mutate: approveDevolutionEPI, isPending: isDevolvingEPI } =
    useApproveDevolutionEPI({
      onSuccess: () => {
      ...
      onError: (error: any) => {
        ...
        toast.error("Erro ao aprovar devolução de EPI:\n" + errorMessage);
        console.log(error);
      },
    });

  const handleApproveRequestEPI = (id: string) => {
    approveDevolutionEPI({ id });
  };

  useEffect(() => {
    setFilteredDevolutions(
      devolutions?.filter((devolution: Devolucao) => !devolution.adminAprovacaoId)
    );
  }, [devolutions]);
  ...
  return (
    ...
    >
      ...
      {/* Cards de devoluções pendentes */}
      <div className="grid grid-cols-3 gap-10 self-center">
        {isGettingDevolution ? (
          <p className="text-white">Carregando devoluções...</p>
        ) : (
          filteredDevolutions?.map((data: Devolucao, index: number) => (
            <div
              key={index}
            ...
            >
              <div className="flex gap-2">
                <img
                  ...
                  src={
                    data.retirada.epi.fotoUrl || "https://via.placeholder.com/150"
                  }
                  alt={data.retirada.epi.nome || "Imagem do EPI"}
                />
              ...
              >
                <Button
                  ...
                  onClick={() => handleApproveRequestEPI(data.id)}
                >
                  {isDevolvingEPI ? "Aprovando..." : "Aprovar devolução"}
                </Button>
              </Stack>
            </div>
          ))
        ...
        {filteredDevolutions?.length === 0 && !isGettingDevolution && (
          <p className="text-white text-xl">
            Nenhuma devolução pendente de aprovação.
          </p>
        )}
      </div>
    </Stack>
