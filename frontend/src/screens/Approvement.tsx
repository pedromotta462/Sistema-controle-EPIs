...
const Approvement = () => {
  ...
  const { mutate: approveRequestEPI, isPending: isRequestingEPI } =
    useApproveRequestEPI({
      onSuccess: () => {
      ...
      onError: (error: any) => {
        ...
        toast.error("Erro ao aprovar retirada de EPI:\n" + errorMessage);
        console.log(error);
      },
    });

  const handleApproveRequestEPI = (id: string) => {
    approveRequestEPI({ id });
  };

  useEffect(() => {
    setFilteredRemovals(
      removals?.filter((removal: Retirada) => !removal.adminAprovacaoId)
    );
  }, [removals]);
    ...
    >
      ...
      <div className="grid grid-cols-3 gap-10 self-center">
          ...
          filteredRemovals?.map((data: Retirada, index: number) => (
            ...
            >
              ...
              >
                ...
                >
                  {isRequestingEPI ? "Aprovando..." : "Aprovar retirada"}
                </Button>
              </Stack>
            </div>
          ))
        ...
        {filteredRemovals?.length === 0 && !isGettingRemovals && (
          <p className="text-white text-xl">Nenhuma retirada pendente de aprovação.</p>
        )}
      </div>
    </Stack>
  );
