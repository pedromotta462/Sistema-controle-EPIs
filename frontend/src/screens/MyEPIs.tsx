  ...
  const { mutate: devolutionEPI, isPending: isRequestingEPI } =
    useDevolutionEPI({
      ...
      onError: (error: any) => {
        ...
        toast.error("Erro ao realizar revolução do EPI:\n" + errorMessage);
        console.log(error);
      },
    });
      ...
      <div className="grid grid-cols-3 gap-10  self-center">
                ...
                <div className="flex flex-col" key={index}>
                  ...
                  <p className="text-[#3984F3] text-sm m-2">
                    ...
                    {data.dataPrevistaDevolucao
                      ? new Date(data.dataPrevistaDevolucao).toLocaleDateString(
                          "pt-BR"
                        )
                      : "N/A"}
                  </p>
        ...
        )}
        {
          filteredRemovals?.length === 0 && (
            <p className="text-white text-xl">Nenhum EPI para devolver.</p>
          )
        }
      </div>
    </Stack>
